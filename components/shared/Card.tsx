...
const Card = ({ listing, hasOrderLink, hidePrice }: CardProps) => {
  ...
  return (
    <div className="group relative flex min-h-[380px] w-full max-w-[400px] flex-col overflow-hidden rounded-xl bg-white shadow-md transition-all hover:shadow-lg md:min-h-[438px]">
      ...
      {isListingCreator && !hidePrice && (
        <div className="absolute right-2 top-2 flex flex-col gap-4 rounded-xl bg-white p-3 shadow-sm transition-all">
          <Link href={`/houses/${listing._id}/update`}>
            <Image src="/assets/icons/edit.svg" alt="edit" width={20} height={20} />
          </Link>
          <DeleteConfirmation houseId={listing._id} />
        </div>
      )}
      ...
      > 
       {!hidePrice && <div className="flex gap-2">
        ...
        <Link href={`/houses/${listing._id}`}>
          <p className="p-medium-16 md:p-medium-20 line-clamp-2 flex-1 text-black">{listing.title}</p>
        </Link>
